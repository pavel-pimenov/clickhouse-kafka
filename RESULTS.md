...
|---|---|---|---|---|---|---|---|---|---|
| **rec/s** | 170 514 | 72 702 | 413 382 | 179 498 | **626 875** | 454 304 | 517 639 | 523 035 | 363 048 | 76 747 | 418 539 | 179 972 | 267 853 | 460 332 | 476 405 | **565 931** | 317 028 |
| **время (с)** | 0.59 | 1.38 | 0.24 | 0.56 | **0.16** | 0.22 | 0.19 | 0.19 | 0.28 | 1.30 | 0.24 | 0.56 | 0.37 | 0.22 | 0.21 | **0.18** | 0.32 |
| RSS (KB) | 14 952 | 19 908 | 21 472 | 23 036 | 26 564 | 41 204 | 43 716 | 45 068 | 48 880 | 48 920 | 49 724 | 50 080 | 51 084 | 53 432 | 53 424 | 54 452 | 54 456 |

### Сводная таблица: все типы сжатия

| Конфигурация | Kafka 3p async+parallel | Ускорение | Redpanda 3p async+parallel | Ускорение |
|---|---|---|---|---|
| none | **626 875** rec/s | **+8.6×** | 267 853 rec/s | +3.5× |
| zstd | 454 304 | +6.2× | 460 332 | +6.0× |
| lz4 | 517 639 | +7.1× | 476 405 | +6.2× |
| snappy | 523 035 | +7.2× | **565 931** | **+7.4×** |
| gzip | 363 048 | +5.0× | 317 028 | +4.1× |

> Ускорение относительно sync+seq baseline (Kafka 72.7K, Redpanda 76.7K).

### Key Takeaways

1. **Kafka async+parallel без сжатия — быстрее всего** (627K rec/s). Компрессия добавляет CPU overhead, не окупается на малых данных.
2. **Redpanda async+parallel+snappy — рекорд 566K**. snappy на Redpanda даёт лучший balance CPU/сеть.
3. **lz4 и snappy близки** (~520K Kafka, ~480-566K Redpanda) — оба лёгкие кодекu.
4. **zstd — хорош на Redpanda** (460K), но отстаёт от snappy/lz4.
5. **gzip — медленнее всех** (CPU-heavy), не подходит для высоких нагрузок.
6. **Kafka async+parallel none: 627K — в 3.7× быстрее CH Native** (171K). Удаление `flush()` + 3 параллельных продюсера = драматический прирост.
